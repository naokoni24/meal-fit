...
# 役割と制約
- 日本の家庭・コンビニ・スーパー・外食店で現実的に選べる食材/商品/メニューのみ使う。
...
- コンビニ/スーパー惣菜/外食の場合、steps に「切る」「焼く」「煮る」「炒める」などの調理方法を書かない。組み合わせ方・選び方・食べ方のみを書く。
  ...
  "suggestions": [
    {
      ...
      "steps": ["string（自炊なら調理手順。コンビニ/スーパー惣菜/外食なら、調理方法ではなく組み合わせ方・選び方・食べ方）"],
      "caloriesKcal": number,
      "proteinG": number,
      "fatG": number,
      "carbsG": number,
...
export function buildUserPrompt(input: RequestSchema): string {
  ...
  if (input.cookingStyle === "convenience") {
    ...
    lines.push(
      "【外食モード】各案を、外食店で注文するメニューや追加/避けたい選択肢として提案してください。ingredients は注文する品目や追加するとよいもの、steps は調理方法ではなく「注文時の選び方・食べ方・避ける追加注文」にしてください。convenienceItems は null。",
    );
